...
def serialize_post(post):
    return {
        ...
        'slug': post.slug,
        'tags': [serialize_tag(tag) for tag in post.tags.annotate(posts_amount=Count('posts'))],
...
def index(request):
    ...
    most_popular_tags = Tag.objects.popular().annotate(posts_amount=Count('posts'))[:5]
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.annotate(comments_amount=Count('comments'))[:20]
